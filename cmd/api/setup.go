...
import (
	...
	c "github.com/vladyslavpavlenko/peparesu/controllers"
	m "github.com/vladyslavpavlenko/peparesu/middleware"
	"github.com/vladyslavpavlenko/peparesu/models"
...
func runDatabaseMigrations(db *gorm.DB) error {
	...
	//err = db.AutoMigrate(&models.WorkingHours{})
	//if err != nil {
	//	return err
	//}
	...
	//err = createInitialWorkingHours(db)
	//if err != nil {
	//	return errors.New(fmt.Sprint("error creating initial working hours:", err))
	//}
...
// createInitialWorkingHours creates initial working hours in the `working_hours` table.
func createInitialWorkingHours(db *gorm.DB) error {
	var count int64

	if err := db.Model(&models.WorkingHours{}).Count(&count).Error; err != nil {
		return err
	}
	...
	initialData := []models.WorkingHours{
		{
			RestaurantID: 1,
			DayOfWeek:    0,
			IsClosed:     false,
